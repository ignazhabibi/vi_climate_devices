        ...
        try:
            ...
            if not getattr(self, "known_devices", None):
                ...
                for gateway in gateways:
                    try:
                        _LOGGER.debug("Processing gateway serial: %s...", gateway.serial[:4] if gateway.serial else "None")
                        # Our Model guarantees installation_id (String)
                        # We use getattr just in case of mock/version mismatches, but prioritize the property
                        inst_id = getattr(gateway, "installation_id", None)
                        
                        # Fallback (just in case API sends data without it, though unlikely with current models)
                        ...
                        if inst_id:
                            ...
                            filtered_devices = [
                                d for d in gw_devices 
                                if d.id not in IGNORED_DEVICES
                            ]
                            ...
                            _LOGGER.warning("Skipping gateway %s (no installation ID)", gateway.serial)
                            
                    except Exception as e:
                        _LOGGER.error("Failed to fetch devices for gateway %s: %s", gateway.serial, e)
            ...
            if self.known_devices:
                ...
                for device in self.known_devices:
                    _LOGGER.debug("Updating device ID: ...%s", str(device.id)[-4:] if device.id else "unknown")
                    try:
                        ...
                        _LOGGER.warning("Failed to update device %s: %s", device.id, err)
...
class ViClimateAnalyticsCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict:
        ...
        # Fallback for users where device_type might be missing/different but they have a device
        if not heating_devices and self.main_coordinator.data:
                # If we have only 1 device total, assume it's the heating one
                if len(self.main_coordinator.data) == 1:
                    heating_devices.append(list(self.main_coordinator.data.values())[0])
        ...
        for device in heating_devices:
            _LOGGER.debug("Fetching analytics for device ID: ...%s", str(device.id)[-4:] if device.id else "unknown")
            ...
            try:
                ...
                _LOGGER.error("Failed to fetch analytics for device %s: %s", device.id, err)
